...
class TestBurger:
   #Проверка работы метода set_buns, добавляющего булочку в бургер.
    def test_set_buns_success(self, mock_bun):
        burger = Burger()
        burger.set_buns(mock_bun)
        assert burger.bun == mock_bun

    #Проверка работы метода add_ingredient, добавляющего ингредиенты в бургер
    #С помощью параметризации выполняем три теста: проверяем по очереди добавление соуса и двух разных начинок.
    ...
    #Проверка работы метода move_ingredient, перемещающего ингредиенты в бургере.
    ...
    #Проверка работы метода get_price, высчитывающего конечную стоимость бургера.
    ...
    #Проверка работы метода get_receipt, получающего рецепт собранного бургера и его стоимость.
